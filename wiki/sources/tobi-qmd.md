...
- Runs locally via `node-llama-cpp` with GGUF models — no external API calls. Reported local models include EmbeddingGemma (300M) for embeddings, Qwen3-Reranker (0.6B) for scoring, and a ~1.7B query-expansion model.
- Ships an MCP server exposing tools such as `query`, `get`, and `multi_get`, for integration with Claude and other agents.
