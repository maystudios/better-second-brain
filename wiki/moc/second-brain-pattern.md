---
type: moc
title: "Second Brain Pattern (Map of Content)"
covers: [topic/llm, topic/pkm]
tags: [moc]
created: 2026-06-19
updated: 2026-06-19
---

# Second Brain Pattern (Map of Content)

This is the navigation hub for the bootstrap domain: how to use an LLM to build and maintain a personal knowledge base, and how a Better Second Brain (BSB) extends that idea. Start with the canonical pattern, then read the concepts, sources, and entities that make up the stack. For the build-time view of how BSB is assembled, see [[wiki/moc/bsb-architecture]].

## Start here

- [[wiki/concepts/llm-wiki-pattern]] — the core pattern: raw / wiki / schema layers and ingest / query / lint operations.
- [[wiki/concepts/rag-vs-llm-wiki]] — why a persistent wiki differs from re-running retrieval per query.
- [[wiki/sources/karpathy-llm-wiki]] — the canonical pattern document.

## Sources

- [[wiki/sources/karpathy-llm-wiki]]
- [[wiki/sources/karpathy-llm-knowledge-bases]]
- [[wiki/sources/safishamsi-graphify]]
- [[wiki/sources/tobi-qmd]]
- [[wiki/sources/obsidian-bases]]

## Concepts

- [[wiki/concepts/llm-wiki-pattern]]
- [[wiki/concepts/rag-vs-llm-wiki]]
- [[wiki/concepts/knowledge-graph-graphrag]]
- [[wiki/concepts/research-discipline]]
- [[wiki/concepts/maps-of-content]]
- [[wiki/concepts/zettelkasten]]
- [[wiki/concepts/memex]]

## Entities

- [[wiki/entities/andrej-karpathy]]
- [[wiki/entities/graphify]]
- [[wiki/entities/qmd]]
- [[wiki/entities/obsidian]]
- [[wiki/entities/tiago-forte]]

## Syntheses

- [[wiki/syntheses/is-bsb-better-than-vanilla-llm-wiki]]

## Sources

- [[wiki/sources/karpathy-llm-wiki]] — https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f
- Tiago Forte, Building a Second Brain (PARA, Maps of Content) — https://fortelabs.com
